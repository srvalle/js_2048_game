...
class Game {
    ...
    adjacentNumbers(row) {
        ...
        while(i < nonZeroNumbers.length){
            ...
            if (i + 1 < nonZeroNumbers.length && nonZeroNumbers[i] === nonZeroNumbers[i + 1]) {
    ...
    moveRight() {
        ...
        for(let i = 0; i < this.board.length; i++){
            ...
            for(let j = 0; j < nonZeroNumbers.length; j++)newRow[this.board[i].length - nonZeroNumbers.length + j] = nonZeroNumbers[j];
    ...
    moveUp() {
        ...
        for(let j = 0; j < this.board[0].length; j++){
            ...
            const column = [];
            for(let i = 0; i < this.board.length; i++)column.push(this.board[i][j]);
            // Filtra os números diferentes de zero
            ...
            for(let i = 0; i < this.board.length; i++)this.board[i][j] = newColumn[i];
        }
        // adiciona um tile se board foi alterado
        if (previousBoard !== JSON.stringify(this.board)) {
            this.addRandomTile();
    ...
    moveDown() {
        ...
        for(let j = 0; j < this.board[0].length; j++){
            ...
            // eles começarão nas posições 2 e 3 (índices)
            for(let i = 0; i < nonZeroNumbers.length; i++)newColumn[this.board.length - nonZeroNumbers.length + i] = nonZeroNumbers[i];
            // Substitui a coluna atual pela nova coluna
            for(let i = 0; i < this.board.length; i++)this.board[i][j] = newColumn[i];
        }
        // adiciona um tile se board foi alterado
    ...
    isGameOver() {
        ...
        if (this.board.some((row)=>row.some((cell)=>cell === 0))) return false;
    ...
    checkWinCondition() {
        for(let r = 0; r < 4; r++){
            for(let c = 0; c < 4; c++)if (this.board[r][c] === 2048) {
                this.status = "win";
                return;
            }
        }
